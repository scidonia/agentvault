...
class GutenbergProcessor:
    ...
    def process_text(self, text: str, metadata: Dict[str, str]) -> Dict:
        ...
        processed = self.bookwyrm.process_text(
            clean_content, chunk_size=CHUNK_SIZE, chunk_overlap=CHUNK_OVERLAP
        )
    ...
    def process_all_texts(self):
        ...
        for url in GUTENBERG_URLS:
            ...
            all_processed.append(
                {"filename": filename, "metadata": metadata, "processed": processed}
            )
    ...
    def create_summary(self, processed_texts: List[Dict]):
        ...
        summary = {"total_texts": len(processed_texts), "texts": []}
        ...
        for item in processed_texts:
            ...
            text_summary = {
                ...
                "total_characters": sum(
                    len(chunk["text"]) for chunk in processed["chunks"]
                ),
                "embedding_dimension": (
                    len(processed["chunks"][0]["embedding"])
                    if processed["chunks"]
                    else 0
                ),
            }
        ...
        print(
            f"Processed {summary['total_texts']} texts with {sum(t['total_chunks'] for t in summary['texts'])} total chunks"
        )
