...
class RAGAgent:
    ...
    def search_node(self, state: AgentState) -> AgentState:
        ...
        for i, result in enumerate(top_results):
            chunk = result['chunk']
            print(f"  {i+1}. {chunk.get('title', 'Unknown')} (similarity: {result['similarity']:.3f})")
    ...
    def answer_node(self, state: AgentState) -> AgentState:
        ...
        for citation in citations:
            context_texts.append(f"From '{citation['title']}' by {citation['author']}: {citation['full_text']}")
        ...
        if len([c for c in citations if c['similarity'] > 0.3]) == 0:
            answer_parts.append("I couldn't find highly relevant information in the available texts to answer your question.")
